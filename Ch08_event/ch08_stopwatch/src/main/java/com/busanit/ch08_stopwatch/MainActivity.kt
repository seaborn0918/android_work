...
class MainActivity : AppCompatActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    stopBtn.setOnClickListener {
      pauseTime =
        chronometer.base - SystemClock.elapsedRealtime() // 멈춘 시간 설정(기준 시간 - 현재의 elapsedRealtime)
      chronometer.stop()
    ...
    resetBtn.setOnClickListener {
      pauseTime = 0L
      chronometer.base = SystemClock.elapsedRealtime() // 기준시간 재설정
      chronometer.stop()
      stopBtn.isEnabled = false
